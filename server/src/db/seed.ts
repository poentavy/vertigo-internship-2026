...
async function insertUsers() {
  ...
  // Add a dedicated admin user for testing
  const adminPasswordHash = await hashPassword("adminpassword");
  const adminUser: UserInsert = {
    username: "admin",
    email: "admin@predict.it",
  ...
  const userValues = Array.from({ length: USER_COUNT }, (_, index) => {
    ...
    return {
      ...user,
      passwordHash,
      balance: 1000, // Standard initial balance
    };
  });
  ...
  const seededUsers: SeededUser[] = insertedUsers.map((user) => ({
    ...
    email: user.email,
    password: user.username === "admin" ? "adminpassword" : SHARED_PASSWORD,
    remainingBalance: user.balance,
  }));

  console.log(`Created ${seededUsers.length} users (including 1 admin).`);
